...
@interface DLImagePickerController ()<TZImagePickerControllerDelegate,UICollectionViewDataSource,UICollectionViewDelegate,UIActionSheetDelegate,UIImagePickerControllerDelegate,UIAlertViewDelegate,UINavigationControllerDelegate> {
...
/*** 配置基本信息 ***/
- (void)configBaseInfo {
    _showTakePhotoBtnSwitch = NO;  ///< 在内部显示拍照按钮;
    _sortAscendingSwitch = YES;     ///< 照片排列按修改时间升序;
    _allowPickingVideoSwitch = NO; ///< 允许选择视频;
    _allowPickingImageSwitch = YES; ///< 允许选择图片;
    _allowPickingGifSwitch = YES;///< 允许选择gif图
    _allowPickingOriginalPhotoSwitch = YES; ///< 允许选择原图;
    _showSheetSwitch = YES; ///< 显示一个sheet,把拍照按钮放在外面;
    _maxCountTF = @"4";  ///< 照片最大可选张数，设置为1即为单选模式;
    _columnNumberTF = @"4"; ////每行展示照片
    _allowCropSwitch = NO; //单选模式下允许裁剪
    _needCircleCropSwitch = NO;// 使用圆形裁剪框
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == _selectedPhotos.count) {
        cell.imageView.image = [UIImage imageNamed:@"me_photo_icon"];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == _selectedPhotos.count) {
        ...
        if ([[asset valueForKey:@"filename"] containsString:@"GIF"] && self.allowPickingGifSwitch) {
            ...
            imagePickerVc.maxImagesCount = self.maxCountTF.integerValue;
...
#pragma mark - 到这里为止
    ...
    [imagePickerVc setDidFinishPickingPhotosHandle:^(NSArray<UIImage *> *photos, NSArray *assets, BOOL isSelectOriginalPhoto) {
        if (self.DlImagePickerAcquirePhotosBlock) {
            self.DlImagePickerAcquirePhotosBlock(photos,assets,isSelectOriginalPhoto);
        }
    }];
...
- (void)refreshCollectionViewWithAddedAsset:(id)asset image:(UIImage *)image {
    ...
    [_collectionView reloadData];
    if (self.DlImagePickerAcquirePhotosBlock) {
        self.DlImagePickerAcquirePhotosBlock(_selectedPhotos,_selectedAssets,_isSelectOriginalPhoto);
    }
}
...
- (void)deleteBtnClik:(UIButton *)sender {
    ...
    //自己添加用于来删除不要的图片
    if (self.DlImagePickerDeletePhotosAtIndexBlock) {
        self.DlImagePickerDeletePhotosAtIndexBlock(sender.tag);
    }
}
